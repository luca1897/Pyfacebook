...
class PostFileRequest(PyFacebook,Request):

	def post_file_request(self,id,**args):
		if "access_token" in args:
			self.specific_access_token = args["access_token"]
		url = "%s%s/photos/?access_token=%s" % (GRAPH_URL,id,self.specific_access_token if self.specific_access_token else generic_access_token)
...
class PostRequest(PyFacebook,Request):	
...
class GetRequest(PyFacebook,Request):	
...
class DelRequest(PyFacebook,Request):	
...
class Feed(PostRequest):
	"""create a link, post or status message"""
	def create_post(self,parameter):
		return self.del_request(comp ="feed" , post =parameter)
			
class Comments(PostRequest,DelRequest):
...
class Likes(PostRequest,DelRequest):
...
class Man_friendlist(PostRequest,DelRequest):	
...
class Man_album(PostRequest):		
	
	def create_album(self,**args):		
		"""create an album for a user"""
		return self.post_request( comp ="albums", post =args)
			
	
class UploadFiles(PostFileRequest):
	...
	def upload_files(self,args):
		...
		for a in args:
			...
			ret.append(self.post_file_request(self.id,body='\r\n'.join(body)))
		
		return ret	
	
	
class Connection(GetRequest):
...
class Application(Object,Connection,PostRequest,DelRequest):	
	...
	def create_account(self,**args):
		...
		return self.post_request( comp ="accounts/test-users" , post =args)
	...
	def create_subscription(self,**args):
		...
		return self.post_request( comp ="subscriptions" , post =args)
	...
	def delete_subscription(self,object=None): # object = {"object","user|permission|page"} If no object is specified all subscriptions are deleted.
		...
		return self.del_request( comp ="subscriptions", param ={"object":object})
		
	
	def create_translation(self,native_strings):
		"""upload application strings for translation"""
		return self.post_request( comp ="translations" , post ={"native_strings":native_strings})

	def delete_translation(self,native_hashes):
		"""delete a translation string"""
		return self.del_request( comp ="translations" , param ={"native_hashes":native_hashes})
	...
	def create_achievement(self,**args): #parameter {"achievement":Unique URL to the achievement}
		...
		return self.post_request( comp ="achievements" , post =args)		
	...
	def delete_achievements(self,achievement): # parameter = {"achievement","The unique URL to the achievement."}
		...
		return self.del_request( comp ="achievements" , param ={"achievement":achievement})		
	...
	def unban_user(self,u_id):
		"""unban a user"""
		return self.del_request( comp ="banned" , get ={"uid":u_id},post="")	
...
class Event(Object,Connection,Feed,DelRequest):	
...
class Page(Object,Connection,Man_event,Feed,Man_note,UploadFiles,Man_album):	
...
class Photo(Object,Connection,Comments,Likes):	
...
class Post(Object,Connection,Comments,Likes):	
...
class Status(Object,Connection,Comments,Likes):
...
class Thread(Object,Connection):
...
class User(Object,Connection,Man_album,Man_event,Feed,Man_friendlist,Likes,Man_note,UploadFiles):
	...
	def create_achievement(self,achievement):
		...
		return self.post_request(comp="achievements",post={"achievement":achievement})
	
	def delete_achievement(self,achievement):
		"""delete an achievement for a user"""
		return self.del_request( comp ="achievements",post={"achievement":achievement})	
	
	def post_score(self,score):	
		"""post a score for a user"""
		return self.post_request(comp="scores",post={"score":score})
	
	def is_liked(self,id):
		"""check if a User likes a specific page"""
		url = "%s%s/likes/%s?access_token=%s" % (GRAPH_URL, self.id ,id,self.specific_access_token if self.specific_access_token else generic_access_token)
		return self.get_request(url)		
	
	def de_authorize_app(self,**args):
		"""de-authorize an application or revoke a specific extended permissions"""
		return self.del_request( comp ="permissions",post=args)			
	
	def is_friend(self,id):			
		"""check if a User is a friend of the current session User"""
		url = "%s%s/friends/%s?access_token=%s" % (GRAPH_URL, self.id ,id,self.specific_access_token if self.specific_access_token else generic_access_token)
		return self.get_request(url)	
	
	def checkin(self,**args):
		"""checkin an user"""
		return self.post_request( comp ="checkins", post =args)	
	
	
class Video(Object,Connection,Likes,Comments):
